...
public class TheSecondaryLevelOfCameraLoadFacade : TheSecondaryLevelOfLoadTreeFacade
{
    ...
    public TheSecondaryLevelOfCameraLoadFacade(GameObject NewTrackObject) : base(NewTrackObject)
    {
    ...
    {
        if (TreatmentObject.CurrentInfo.TestLoadTree != 678910)
        {
            TreatmentObject.CurrentInfo.ChangeTestLoadTree(687910);
        }

        if (!TreatmentObject.CurrentInfo.currentSaveInfo.IsTreatmentActive)
        {
           TreatmentObject.CurrentInfo.ChangeIsActiveTreatmentStateBool(true);
        }
    }
